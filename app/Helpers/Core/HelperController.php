...
use App\Helpers\Constants\CommonValidations as V;
use App\Helpers\Constants\Messages;
use App\Helpers\Constants\ResponseType;
use App\Helpers\Transform\Arrays;
...
class HelperController extends Controller
{
    ...
    public function respond($data = [], array $errors = [], string $view = 'backend.dashboard', string $message = Messages::OK, int $statusCode = ResponseType::OK, array $headers = [], array $paginationParams = [])
    {
        if (self::isAPI()) {
            if (!is_array($data)) {
                $data = $data->toArray();
            ...
            return view('backend.dashboard')->withErrors($errors);
        }
